...
class DeepSpeedRLHFEngine():
    ...
    def __init__(self, actor_model_name_or_path, critic_model_name_or_path,
        ...
        hidden_dim = self.actor.module.config.hidden_size
        intermediate_dim = self.actor.module.config.intermediate_size
        self.ICM = self._init_icm(hidden_dim, intermediate_dim)
    ...
    def _init_actor(self, actor_model_name_or_path):
        ...
        if self.args.actor_lora_dim > 0:
            actor_model = convert_linear_layer_to_lora(
                ...
                self.args.actor_lora_dim, self.args.actor_lora_scaling, self.args.actor_lora_dropout)
    ...
    def _init_critic(self, critic_model_name_or_path):
        ...
        if self.args.critic_lora_dim > 0:
            critic_model = convert_linear_layer_to_lora(
                ...
                self.args.critic_lora_dim, self.args.critic_lora_scaling, self.args.critic_lora_dropout)
            if self.args.only_optimize_lora:
                critic_model = only_optimize_lora_parameters(critic_model)
                critic_model.rwtransformer = make_model_gradient_checkpointing_compatible(
                    critic_model.rwtransformer)
    ...
    def _init_icm(self, hidden_dim, intermediate_dim):
        stime = log_init("ICM")

        # DS Config
        ds_config = get_train_ds_config(
            offload=self.args.offload_icm_model,
            dtype=self.args.dtype,
            stage=self.args.icm_zero_stage,
        ...
        # Model
        icm_model = ICM(hidden_dim, intermediate_dim)

        # Optimizer
        AdamOptimizer = DeepSpeedCPUAdam if ds_config["zero_optimization"]["offload_optimizer"]["device"] == "cpu" else FusedAdam
        optim_params = get_optimizer_grouped_parameters(
            icm_model, self.args.actor_weight_decay,
            self.args.actor_lora_learning_rate)
        optim = AdamOptimizer(optim_params,
                              lr=self.args.icm_learning_rate,
                              betas=(0.9, 0.95))
        ...
        # DeepSpeed Engine
        #TODO: move enable_hybrid_engine and pin_parameters to ds_config
        icm_engine, *_ = deepspeed.initialize(model=icm_model,
                                                optimizer=optim,
                                                lr_scheduler=lr_scheduler,
                                                config=ds_config)

        log_init("ICM", stime=stime)
