...
def KMP_search(A, B):
    ...
    n = len(A)
    m = len(B)
    lps = build_partial_match_table(B)
    i = 0  # index for A
    j = 0  # index for B
...
class SelfBLEURewardFunction():
    def __init__(self, tokenizer, sample_size):
        self.tokenizer = tokenizer
        self.sample_size = sample_size
        self.references = []
        self.pad_token_id = tokenizer.pad_token_id
    
    def add_sample(self, sample: torch.Tensor):
        # remove padding tokens from the sample
        sample = [s[s != self.pad_token_id].tolist() for s in sample]
        self.references.extend(sample)
    
    def compute_bleu(self, hypothesis: torch.Tensor):
        if self.sample_size > 0:
            sample_size = min(self.sample_size, len(self.references))
            references = random.sample(self.references, k=sample_size)
        else:
            references = self.references
        
        bleu = BLEU(references, weights={4: (1/4, 1/4, 1/4, 1/4)})
        # remove padding tokens from the hypothesis, hypothesis dimension is (bsz, token)
        hypothesis = [h[h != self.pad_token_id].tolist() for h in hypothesis]
        scores = list(bleu.get_score(hypothesis).values())[0]
