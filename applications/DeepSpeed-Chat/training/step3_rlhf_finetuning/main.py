...
def parse_args():
    ...
    parser.add_argument("--actor_lora_scaling",
                        type=int,
                        default=1)
    parser.add_argument("--actor_lora_dropout",
                        type=float,
                        default=0.)
    ...
    parser.add_argument("--critic_lora_module_name",
                        ...
                        default="decoder.layers.",
                        help="The scope of LoRA.")
    parser.add_argument("--critic_lora_scaling",
                        type=int,
                        default=1)
    parser.add_argument("--critic_lora_dropout",
                        type=float,
                        default=0.)
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Sample size used to compute SelfBLUE.")
    parser.add_argument(
        "--cdrlhf_topk",
        type=int,
        default=1)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch_prompt in enumerate(tqdm(prompt_train_dataloader, total=len(prompt_train_dataloader), disable= not (torch.distributed.get_rank() == 0))):
            ...
            if exp_dataset is not None:
                ...
                print_rank_0(
                    f'Epoch: {epoch} | Step: {step} | PPO Epoch: {ppo_ep+1} | Actor Loss: {actor_loss_sum/inner_iter} | Critic Loss: {critic_loss_sum/inner_iter} | ICM Loss: {icm_loss_sum/inner_iter}', args.global_rank)
                ...
                # print_rank_0(
                #     f"Average reward score: {log_metrics['reward']/inner_iter} | EMA reward score: {ema_reward_score.get()} | Average KL: {average_kl} | Average Response Length: {resp_length} | Averaged intrinsic reward {average_intrinsic_reward/inner_iter}",
                    # args.global_rank)
                print_str = ' | '.join([f"{key}: {value}" for key, value in log_metrics.items()])
                print_rank_0(f"EMA reward score: {ema_reward_score.get()} | {print_str}", args.global_rank)
                print_rank_0(
                    "-------------------------------------------------------------------------------------",
                    args.global_rank)
                ...
                if args.enable_tensorboard and torch.distributed.get_rank(
                    ...
                    writer.add_scalar('critic_loss_sum',
                                      ...
                                      global_step=all_micro_step)
                    writer.add_scalar('icm_loss_sum',
                                      icm_loss_sum / inner_iter,
                                      global_step=all_micro_step)
                    for k in log_metrics:
